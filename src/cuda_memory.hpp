...
public:
    ...
    basic_pointer(pointer ptr_ = nullptr):
        ptr{ptr_}
    {}
    basic_pointer& operator=(std::nullptr_t){
        ptr = nullptr;
        return *this;
    }
    operator bool()const{return static_cast<bool>(ptr);}
    operator basic_pointer<const T>()const{return basic_pointer<const T>{ptr};}
...
template<typename T>
auto operator==(const basic_pointer<T>& lhs, const basic_pointer<T>& rhs){return lhs.get() == rhs.get();}
template<typename T>
auto operator!=(const basic_pointer<T>& lhs, const basic_pointer<T>& rhs){return !(lhs == rhs);}
template<typename T>
auto operator-(const basic_pointer<T>& lhs, const basic_pointer<T>& rhs){return lhs.get() - rhs.get();}
template<typename T, typename U>
auto operator+(const basic_pointer<T>& lhs, const U& rhs){return basic_pointer<T>{lhs.get() + rhs};}
template<typename T, typename U>
auto operator+(const U& lhs, const basic_pointer<T>& rhs){return rhs+lhs;}
template<typename T, typename U>
auto operator-(const basic_pointer<T>& lhs, const U& rhs){return lhs+-rhs;}

template<typename T>
auto distance(const basic_pointer<T>& begin, const basic_pointer<T>& end){return end-begin;}

template<typename T>
auto ptr_to_void(const basic_pointer<T>& p){return static_cast<std::conditional_t<std::is_const_v<T>,const void*,void*>>(p.get());}
template<typename T>
auto ptr_to_void(const T* p){return static_cast<const void*>(p);}
template<typename T>
auto ptr_to_void(T* p){return static_cast<void*>(p);}

template<typename> constexpr bool is_basic_pointer_v = false;
template<typename T> constexpr bool is_basic_pointer_v<basic_pointer<T>> = true;
...
template<typename T>
void copy(const T* first, const T* last, basic_pointer<T> d_first){
...
void copy(It first, It last, basic_pointer<typename std::iterator_traits<It>::value_type> d_first){
...
template<typename T>
void copy(basic_pointer<T> first, basic_pointer<T> last, std::remove_const_t<T>* d_first){
...
void copy(basic_pointer<T> first, basic_pointer<T> last, It d_first){
    static_assert(!std::is_pointer_v<It>);
...
template<typename T>
void copy(basic_pointer<T> first, basic_pointer<T> last, basic_pointer<std::remove_const_t<T>> d_first){
